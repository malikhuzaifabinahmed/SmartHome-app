...
export default function DeviceForm({ device, home }) {
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="flex flex-col gap-5 mb-10 w-full max-w-lg mx-auto bg-white p-6 rounded-lg ">
      {!isLoading && <div className="flex flex-col gap-5">
        ...
        <div className="grid w-full gap-1.5">
          ...
          <MyButton disabled={isLoading} type='button' variant="secondary" onClick={handleAddProperty}>
            Add Property
          </MyButton>
        </div>

      </div>}
      {isLoading && <div className="text-center text-[2.5rem] font-fraunces_bold text-gray-900 mb-8">
        New Device is Being Created
        <RotateCw className="mx-auto size-10 animate-spin" />

      </div>
      }
      <MyButton disabled={isLoading} type="submit" className="self-center">
